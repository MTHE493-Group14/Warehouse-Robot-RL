...
class AgentLFA:
    ...
    ----------
    lfa : LinearFA
        The regression model used for function approximation.
    ...
    def __init__(self, filename, overwrite):
        ...
        """
        self.lfa = LinearFA(filename, overwrite)
        self.performance = Performance('fa', filename, overwrite)
        return
    ...
    def greedy_policy(self, current_state):
        ...
        """
        X = self.lfa.combine_features_all_actions(current_state.features2())
        argmin_pred = np.argmin(self.lfa.predict(X))
        a = Actions()
        a.set_by_enum(argmin_pred)
        return a
