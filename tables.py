...
class Tables:
    ...
    def __init__(self):
        ...
        self.qvals = np.ones((num_states, num_actions)) * 10
        self.visits = np.zeros((num_states, num_actions))
        self.same_locs = np.concatenate((np.ones((1, num_states)), np.zeros((num_actions-1, num_states)))).transpose()
        return
    ...
    def update(self, s1, s2, a, r):
        ...
        a : Actions
            The actions taken.
        r : int
            The reward recieved at that time step.
        ...
        if not same_locs:
            ...
            self.qvals[s1num][anums] += LEARNING_RATE*(r + DISCOUNT_FACTOR*(min_val) - old_vals)
